...
\end{align*}
\begin{remark}
    The $\mu$-density of a probaility measure is not uniquely defined, however in \cite{Gneiting} a way to uniquely determine a certain $p$ for some $P$ in $\mathcal{L}_\alpha$, when needed, is referenced
\end{remark}
...
Because of its simplicity, the logarithmic score is widely used in various different settings. A particular and also possibly problematic use case of the logarithmic score was described in \cite{Candela} and \cite{Kohonen}. In this particular case, a variation of the logarithmic score was used in a so called PASCAL challenge to score predictions given by an algorithm in the form of a density function. In this challenge, the predictive density was used to estimate the relation between two variables. Utilizing the logarithmic score in this scenario, however, came with a problem. If it was reasonable to expect some point on the continuous space of possible outputs for the given input to have a point mass (a probability greater than $0$), one could pick a small interval aroud this value and set the density on this interval as shown in Fig. 2.1. Since the logarithmic score only depends on the value at the observed point $\omega$, hitting the interval with almost arbitrary high density would result in a really high score even if one takes the average over multiple observations.    
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/Graphic Kohonen Suomela.png}
    \caption{Figure 8 of \cite{Kohonen} shows how small intervals can have really high densities without altering the according CDF very much}
    \end{figure}
\section{Scoring rules for CDF}
As seen in the previous chapter, only relying on predictive densities could be a problem, like in the example of the PASCAL challenge. Problems like this could be prevented if CFD's were used instead of predictive densities. Besides that using a CFD would also allow one to use discrete measures on sample sets to easier represent the predictive distribution as shown in Fig 2.2. Using the logarithmic score with the same representation would result in a "infinit penalty" if a value outside the sample materializes.
\begin{figure}[!h]
            \centering
...
\begin{definition}[CRPS]
    Let $\mathcal P$ be the set of Borel probability measures (measures defined on any open set) on $\mathbb R$. Then we define the Continuous Ranked Probability Score as \begin{align*}
        \text{CRPS}=-\int_{-\infty}^\infty (F(y)-\mathbbm 1_{\{y\geq x\}})^2 dy
    \end{align*}
\end{definition}
Fig. 2.3 shows that the CRPS can be interpreted as a way of measuring the space between the predictive CDF and the and the point measure at the spot of the value that materialized 
\begin{remark}
...
After focussing strongly on the CRPS it remains to be said that the CRPS has the downside, that the integral, which is needed to compute the score is not that easy to calculate. Therefore, as stated in \cite{Gneiting} one might have to resort to using numerical methods to calculate it. Possible other scores that can be considered instead of the CRPS can be the so called energy score, which is a generalization  of the CRPS on $\mathbb R^n$ or scores based only on first and second moments of distributions.
%\chapter{Preliminaries}
%\section{Mathematical Background}
\chapter{Kernel scores}
In this chapter we look at the question of how can be possible to easily define scoring functions by using so called kernels and look at some examples of using those kernels to define some of the scores we already know. 
...
\begin{theorem}
Let $\Omega$ be a Hausdorff space and let $g\colon\Omega\times\Omega\to\mathbb R$ be a non-negative and continuous negative definite kernel. If now $P$ is a Borel probability measure on $\Omega$ and $X,X'$ are independend with distribution $P$ the expression \begin{align*}
    S(P,x)=\frac 12 E_P(g(X,X'))- E_P[g(X,x)]
\end{align*}
is a proper scoring rule regarding the class of Borel probability measures on $\Omega$
\end{theorem}
The proof of the Theorem is outlined in \cite{Gneiting}.
\begin{example}
    Take $g(x,x')=\lvert x-x'\rvert$ as a negative kernel. Using the theorem, we get the alternative definition of the CRPS
\end{example}
\begin{example}
...
\end{align*}
Under certain demands at $\mu$ this scoring rule is strictly proper relative to the class of borel probability measures on $\mathbb R^m$.
\chapter{Scoring rules for quantiles}
In the previous chapters, we looked at scoring rules for given density or cumulative distribution function. In some cases, however, it might not be useful to look at a whole distribution and it might be enough to know where certain sections of the distribution lie. This motivates the following definition: 
...
\begin{theorem}[Thm 6 in \cite{Gneiting}]
    ...
    \end{align*} 
    is proper for predicting a single quantile $\alpha\in(0,1)$
\end{theorem}
\begin{proof}
    Assume $r<q$, then \begin{align*}
        ...
        &=\int\alpha s(r)-\alpha s(q)+(s(x)-s(q))\mathbbm 1_{\{x\leq q\}}-(s(x)-s(r))\mathbbm 1_{\{x\leq r\}} dP(x)
    \end{align*}
    Dividing the integral into parts over the sets $(-\infty,r],(r,q),[q,\infty)$ gives that the ebove expression is equal to \begin{align*}
        \int_{(r,q)}s(x) dP(x)+s(r)P(r)-\alpha s(r)&\leq\int_{(r,q)}s(r) dP(x)+s(r)P(r)-\alpha s(r)\\
        &=s(r) P(q)-s(r)P(r)\\
        &=0,
    \end{align*}
    where the inequality holds because $r<q$ and because $s$ is nondecreasing. The first equality holds because $P$ is some kine of Borel measure.
    For $r>q$ the result is the the same, concluding the proof.
\end{proof}
